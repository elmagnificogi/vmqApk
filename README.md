...
>https://elmagnifico.tech/2023/05/04/Vmq-Android/
>
...
- v2.1(2023.05.10)
  - 修复支付宝积分提示不同导致的无法识别支付

- v2.0 (2022.12.28) [zwc456baby](https://github.com/zwc456baby)主要修改增加几项功能：

  - 支持开机自启(国内定制系统需手动开启自启权限)

   - 支持post失败后再重试一次(这次重试是强制亮屏的)

  - ios设备可以找一台安卓设备-》绑定店员进行收款通知了

  - 优化了里面大量的 okhttp 的访问，做成了单例,防止某些情况下可能导致的内存溢出、错误崩溃等等

  - 增加了一次网络重试（没有做三次、四次是防止重复提交导致订单出问题），而且是唤起到前台的网络重试，（之所以唤起到前台，是测试过小米等机型，如果app长时间在后台（6小时以上），app会被限制网络，除非启动前台Service或者Activity
